...
is deprecated (meaning most of those constructs will be gradually removed), in 
favor of adding ad hoc statements of the type (fully left-adjusted, to stand out):::

  write (stderr,*) wallclock(), mpi%rank, omp%thread, 'message', tsak%id
...
so they are recoverable via a simple ``git revert``, w/o introducing outher changes.
