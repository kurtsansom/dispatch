!> =============================================================================
!> Opacity module based on the Semenov et al (2003) code.   
!> =============================================================================
      MODULE opacity_mod
      USE scaling_mod
      implicit none
...
C Global variable(s):
      ...
      CONTAINS
C> =============================================================================
      FUNCTION mean (self, rho, T) RESULT (kap)
      class (opacity_t):: self
      real rho(:), T(:)
      real kap(size(rho))
      real(8) rho8, T8, kap8, c
      integer i
C
!$omp critical (opac_cr)
      if (first_time) then
        first_time = .false.
        call self%init
      end if
!$omp end critical (opac_cr)
      c = scaling%m/scaling%l**2
      do i=1,size(rho)
        rho8 = rho(i)/scaling%d
        T8 = T(i)/scaling%temp
        CALL cop(eD,eG,rho8,T8,kap8)
        kap(i) = kap8*c
      end do
      END FUNCTION
C> =============================================================================
...
C
C Open input file:
      OPEN (unit=07,file='opacity/opacity.inp',status='old',
     &      access='sequential')
C Read input file:
      read(07,'(a3)') model
      read(07,'(a1)') top
...
C For what kind of opacity calculations will be performed:
      if (ross) then
         open(unit=99,file='opacity/kR.out',status='unknown',
     &        access='append')
         print  *, 'Calculation of Rosseland mean opacities '        
         write(99,*), 'Calculation of Rosseland mean opacities '        
      else
         open(unit=99,file='opacity/kP.out',status='unknown',
     &        access='append')      
...
C Local variable(s):
      ...
      PARAMETER (kR_H='opacity/kR_h2001.dat', !a file with Rosseland mean gas opacity,
     &           kP_H='opacity/kP_h2001.dat') !a file with Planck mean gas opacity,
      DIMENSION seq(71*71)    
C      
C Open input file: 
      if (ross) then
