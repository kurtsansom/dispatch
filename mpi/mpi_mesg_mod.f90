...
MODULE mpi_mesg_mod
  ...
  type, public:: mpi_mesg_t
    ...
    type(mesg_list_t):: sent_list, recv_list
    integer:: n_check=0
    integer:: n_ready=0
    integer:: n_update=0
    integer:: n_send=0
    ...
    logical:: recv_priv=.true.
    logical:: recv_active=.true.
    logical:: send_priv=.true.
    logical:: debug=.false.
    logical:: queue_unpack=.true.
    logical:: uniq_mesg=.false.
    real:: delay_ms=0.0
    real:: test_time=0.0
  contains
    procedure:: init
...
SUBROUTINE check_priv
  ...
  do while (associated(mesg))
    next => mesg%next                                         ! in case removed
    !call MPI_TESTALL (mesg%nreq, mesg%reqs, flag, MPI_STATUSES_IGNORE, ierr)
    call mesg%test_all (flag)
    if (flag) then
      call sent_list%remove (mesg)                            ! yes, remove
...
SUBROUTINE test_all (self, flag)
  ...
  if ((self%nreq<=0) .or. (.not.associated(self%reqs))) then
    print*,'WARNING: test_all', self%nreq, associated(self%reqs)
    return
  end if
#ifdef MPI
  if (mpi%mode == MPI_THREAD_MULTIPLE) then
...
SUBROUTINE init (self)
  ...
  namelist /mpi_mesg_params/ min_nq, max_sent, max_probe, max_recv, every_recv, &
                             ...
                             recv_active, recv_priv, queue_unpack, test_time, &
                             uniq_mesg, debug, verbose
  integer:: iostat
  !-----------------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------------
  recv_priv = self%recv_priv
  send_priv = self%send_priv
  recv_active = self%recv_active
  queue_unpack = self%queue_unpack
  !-----------------------------------------------------------------------------
  ! Namelist input
  !-----------------------------------------------------------------------------
  ...
  self%recv_priv    = recv_priv
  self%recv_active  = recv_active
  self%queue_unpack = queue_unpack
  !-----------------------------------------------------------------------------
...
contains
  ...
  subroutine probe_for
    ...
    if (flag) then
      ...
      mesg%req = req
      mesg%id = stat(MPI_TAG)
      if (mpi_mesg%uniq_mesg) then
        mesg%seq = mod(mesg%id,100)
        mesg%id = mesg%id/100
      end if
      mesg%sender = stat(MPI_SOURCE)
...
!===============================================================================
!> Return true or false, depending on if the (send!) mesg request is complete or not
!===============================================================================
...
SUBROUTINE irecv (self, rank, id)
  class(mesg_t):: self
  integer:: id
  integer:: rank, tag, ierr
  !-----------------------------------------------------------------------------
  ! Construct a unique tag, if so requested, and otherwise use the given id.
  ! Use the next seq number, relative to the one just received.
  !-----------------------------------------------------------------------------
  self%id = id
  if (mpi_mesg%uniq_mesg) then
    self%seq = self%seq+1
    tag = mod(self%seq,100) + 100*self%id
  else
    tag = self%id
  end if
  self%tag = tag
...
SUBROUTINE delay (self, n, ms)
  ...
  if (ms_l==0.0) return
  call trace%begin ('mpi_mesg_t%delay', itimer=itimer)
  call mpi%delay (ms_l)
  if (present(n)) then
    write (io_unit%log,*) wallclock(), omp%thread, 'delay:', n
    flush (io_unit%log)
  end if
  !$omp atomic
