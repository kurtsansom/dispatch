    ...
    do iy=m(2)%lb,m(2)%ub
      ...
      do ix=m(1)%lb,m(1)%ub
        ...
        ff(ix,iy,iz,idx%e ) = d0*csound**2/(gamma*(gamma-1)) + &
          half*ff(ix,iy,iz,1)*(ff(ix,iy,iz,2)**2+ff(ix,iy,iz,3)**2+ff(ix,iy,iz,4)**2)
        if (self%mhd) then
...
SUBROUTINE turbulence (self, m, ff, idx)
  ...
  if (first_time) then
    ...
    if (io%master) write (io%output, IC_turbulence_params)
  end if
  !$omp end critical (input_cr)
  ff(:,:,:,idx%d        ) = 1.0
  ff(:,:,:,idx%e        ) = 1.0
  if (gamma==1.0) then
    ff(:,:,:,idx%e        ) = 1.0 + 0.5*(b0(1)**2+b0(2)**2+b0(3)**2)
  else
    ff(:,:,:,idx%e        ) = 1.0/(gamma-1.) + 0.5*(b0(1)**2+b0(2)**2+b0(3)**2)
  end if
